...
// Unit tests for the macOS native implementation of FlutterGetAppGroupDirectoryPlugin.
class RunnerTests: XCTestCase {
  ...
  func testGetInvalidGroupId_returnsAppGroupNotFoundError() {
    let plugin = FlutterGetAppGroupDirectoryPlugin()
    // An arbitrary string that is not a registered App Group on this machine
    ...
    let resultExpectation = expectation(description: "result block must be called.")
    plugin.handle(call) { result in
      // On a machine without this group configured the result is a FlutterError
